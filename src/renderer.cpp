...
static std::vector<size_t> colWidths(const std::vector<std::string>& headers,
    ...
    for (size_t j = 0; j < headers.size(); ++j) w[j] = headers[j].size();
    for (const auto& r : rows) {
        for (size_t j = 0; j < headers.size(); ++j) {
            if (j < r.size()) w[j] = std::max(w[j], r[j].size());
        }
    }
    return w;
}
...
static void printOneRow(const std::vector<std::string>& row,
    ...
    for (size_t j = 0; j < w.size(); ++j) {
        ...
        os << ' '
           << std::left << std::setw(static_cast<int>(w[j])) << cell
