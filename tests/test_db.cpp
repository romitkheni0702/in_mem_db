...
static void run_all_sql(const std::string& sql, Database& db) {
    ...
    for (const auto& st : stmts)
        ex.execute(st); // pass the variant directly
}

TEST_CASE("CREATE + INSERT + SELECT * basic flow") {
    Database db;
    const std::string sql = "CREATE TABLE users (id int, name str);"
...
TEST_CASE("INSERT fills defaults for missing columns") {
    Database db;
    run_all_sql("CREATE TABLE t (n int, s str);"
                "INSERT INTO t (s) VALUES (\"x\");",
                db);
...
TEST_CASE("WHERE with '=' and '!=' works") {
    Database db;
    run_all_sql("CREATE TABLE t (id int, name str);"
...
TEST_CASE("Parse errors: lowercase keyword is invalid (case-sensitive)") {
    ...
    REQUIRE_THROWS_AS(run_all_sql("CREATE TABLE t (id int, name str);"
                                  "select * FROM t;",
                                  db),
                      std::runtime_error);
}
...
TEST_CASE("Inequality operators <, <=, >, >=") {
    ...
    // ints
    {
        std::string sql = "CREATE TABLE nums (n int, label str);"
    ...
    // n < 3 -> 1 and 2
    {
        Parser p("SELECT n FROM nums WHERE n < 3;");
        auto v = p.parseAll();
        Executor ex(db);
        std::ostringstream cap;
        auto* old = std::cout.rdbuf(cap.rdbuf());
    ...
    // strings: label >= "b" -> b, c
    {
        Parser p("SELECT label FROM nums WHERE label >= \"b\";");
        auto v = p.parseAll();
        Executor ex(db);
        std::ostringstream cap;
        auto* old = std::cout.rdbuf(cap.rdbuf());
...
TEST_CASE("Inequalities: <, <=, >, >=") {
    ...
    std::string setup = "CREATE TABLE t (id int, name str);"
